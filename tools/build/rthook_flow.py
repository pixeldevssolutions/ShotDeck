"""PyInstaller runtime hook: the two things flow.sh does for a source run
...
# Same file flow.sh sources; KEY=value lines, # comments, optional quotes.
_ENV_FILE = os.environ.get("FLOW_ENV_FILE", "/etc/sgdesk/sgdesk.env")
try:
...
if __name__ == "__main__" and not getattr(sys, "frozen", False):
    # python tools/build/rthook_flow.py -- checks the env-file parser
    ...
    sample = ("# comment\n"
              ...
              'FLOW_REZ_REQUEST="python-3.11 PySide6"\n'
              "  SPACED  =  value  \n"
              "novalue\n")
    with tempfile.NamedTemporaryFile("w", suffix=".env", delete=False) as fh:
        fh.write(sample)
        path = fh.name
    for key in ("SG_SCRIPT_KEY", "SG_URL", "FLOW_REZ_REQUEST", "SPACED"):
        os.environ.pop(key, None)
    os.environ["FLOW_ENV_FILE"] = path
    ...
    assert os.environ["FLOW_REZ_REQUEST"] == "python-3.11 PySide6"
