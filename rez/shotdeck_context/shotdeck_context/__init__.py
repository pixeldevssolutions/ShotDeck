"""Read the ShotGrid context that ShotDeck handed to this DCC.

Publish tools inside Maya, Nuke, Blender and so on should import this rather
than reading SHOTDECK_* environment variables directly, so the context file
format can change in one place.
    ...
    if ctx.task_id:
        ...
        warn("Launched without a task — pick one in ShotDeck first.")
"""
...
CONTEXT_ENV_VAR = "SHOTDECK_CONTEXT_FILE"
...
def get(refresh=False):
    ...
    when the app was not launched from ShotDeck, so tools can degrade rather
...
def _from_env():
    """Fallback built from the flat SHOTDECK_* variables."""
    def num(name):
        raw = os.environ.get(name, "")
        return int(raw) if raw.isdigit() else None

    if not os.environ.get("SHOTDECK_PROJECT_ID"):
        return {}
    return {
        "schema": 1,
        "site": os.environ.get("SHOTDECK_SITE", ""),
        "user": {
            "login": os.environ.get("SHOTDECK_USER", ""),
            "email": os.environ.get("SHOTDECK_USER_EMAIL", ""),
        },
        "project": {
            "id": num("SHOTDECK_PROJECT_ID"),
            "name": os.environ.get("SHOTDECK_PROJECT_NAME", ""),
            "code": os.environ.get("SHOTDECK_PROJECT_CODE", ""),
        },
        "software": {
            "code": os.environ.get("SHOTDECK_SOFTWARE", ""),
            "version": os.environ.get("SHOTDECK_SOFTWARE_VERSION", ""),
        },
        "task": {
            "id": num("SHOTDECK_TASK_ID"),
            "name": os.environ.get("SHOTDECK_TASK_NAME", ""),
            "step": os.environ.get("SHOTDECK_STEP", ""),
            "entity_type": os.environ.get("SHOTDECK_ENTITY_TYPE", ""),
            "entity_id": num("SHOTDECK_ENTITY_ID"),
            "entity_name": os.environ.get("SHOTDECK_ENTITY_NAME", ""),
        } if num("SHOTDECK_TASK_ID") else None,
    }
