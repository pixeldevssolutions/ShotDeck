...
POOL = os.environ.get("SHOTDECK_DEADLINE_POOL", "none")
GROUP = os.environ.get("SHOTDECK_DEADLINE_GROUP", "none")
PRIORITY = os.environ.get("SHOTDECK_DEADLINE_PRIORITY", "50")
CHUNK_SIZE = os.environ.get("SHOTDECK_DEADLINE_CHUNK", "1")
...
def frames(adapter):
    ...
    if not span:
        raise DeadlineError(
            "ShotDeck could not read a frame range from this scene, and will "
...
def job_info(scene, plugin_name, frame_string, ctx=None, description=""):
    ...
    info = {
        ...
        "Comment": description or "Submitted from {0} by ShotDeck".format(
...
def plugin_info(scene, adapter, ctx=None):
    ...
    if ctx.software_version:
        ...
        # wants major.minor -- which is the front of what ShotDeck launched.
...
def submit(adapter, ctx=None, description=""):
    ...
    folder = tempfile.mkdtemp(prefix="shotdeck-deadline-")
