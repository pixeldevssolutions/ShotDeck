"""Houdini adapter: the ShotDeck menu on Houdini's main menu bar.
...
def message(text):
    ...
    if hou.isUIAvailable():
        hou.ui.displayMessage(text, title="ShotDeck")
...
def ask_path(start_dir, extension, suggested=""):
    ...
    chosen = hou.ui.selectFile(
        start_directory=start_dir, title="ShotDeck: Save As",
...
def deadline_plugin_info(scene):
    ...
    if rop is None:
        ...
        raise deadline.DeadlineError(
            ...
            "ShotDeck will not pick one for you.")
