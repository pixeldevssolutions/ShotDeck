...
def install():
    ...
    menu = type("SHOTDECK_MT_menu", (bpy.types.Menu,), {
        "bl_idname": MENU_ID,
        "bl_label": MENU_NAME,
        "draw": draw,
    })
...
def _operator(bpy, module, label, attr):
    ...
    return type("SHOTDECK_OT_" + attr, (bpy.types.Operator,), {
        "bl_idname": "shotdeck." + attr,
        "bl_label": label,
        "bl_options": {"REGISTER"},
        "execute": execute,
    })
...
def message(text):
    ...
    bpy.context.window_manager.popup_menu(draw, title="ShotDeck", icon="INFO")
...
def action_save_as():
    ...
    trade is that ShotDeck does not get to comment on where the file landed --
