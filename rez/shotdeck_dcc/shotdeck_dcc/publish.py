...
media inspection, upload, notes -- stays in ShotDeck's publish dialog, which
already owns it.
...
os.environ. It is deliberately not part of the SHOTDECK_* context. Anything
...
def prepare(ctx=None):
    ...
    if os.path.exists(publish_path):
        ...
        raise PublishError(
            ...
            "have published outside ShotDeck. Check with them before "
...
def publish(adapter, ctx=None, description=""):
    ...
    try:
        ...
        registration_error = RuntimeError(
            ...
            "published on disk. Register it from ShotDeck, or ask a "
