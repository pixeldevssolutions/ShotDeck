...
Deliberately narrow. Flow's own publish dialog owns media publishing --
...
API_PATH_VAR = "FLOW_SG_API_PATH"


def api():
    """The shotgun_api3 module, or None.

    A DCC ships its own Python and cannot see Flow's venv, so the API is
    normally missing here. Flow exports FLOW_SG_API_PATH pointing at
...
def connect():
    ...
    if shotgun_api3 is None:
        raise NotConfigured(
            ...
            "Flow at launch, so this session was either started outside "
            "Flow or predates that export.".format(
                API_PATH_VAR,
    ...
    if not ctx.site:
        raise NotConfigured("FLOW_SITE is not set, so there is no site to "
