...
def launch(project, software, login=None):
    ...
    if software.get("sg_external_ui"):
        return _launch_external_ui(project, software, login)

    args = shlex.split(software.get("linux_args") or "")
    env = build_env(project, software, extra={"SGDESK_USER": login or ""})
...
def _launch_external_ui(project, software, login):
